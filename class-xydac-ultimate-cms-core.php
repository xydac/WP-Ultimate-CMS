...
{
    public $parent_class; // The main class that uses object of this class
    public $type; //used in parent class;
    public $field_val; //this has the name of type of which field is being edited.

    public $xydac_core_name;
    public $xydac_core_label;
    public $xydac_core_form_action;
    public $xydac_core_form_array;
    public $xydac_core_editmode = false;
    public $xydac_editdata;
    public $xydac_core_message = "";
    public $xydac_core_error = "";
    public $option_name;
    public $option_value;
    public $baselink;
    public $activation;
    public $namefield_name;
    public $xydac_core_show_additional; //Is used to show/hide additional form elements on left side.
    public $xydac_core_show_left; //Is used to show/hide left side.
    public $xydac_core_show_doaction; //Is used to show/hide left side.
    public $xydac_core_show_delete; //Is used to show/hide delete row action.
    public $xydac_core_show_sync; //Is used to show/hide sync rowaction.
    public $xydac_core_show_fields; //Is used to show/hide fields rowaction.
    public $xydac_core_show_export; //Is used to show/hide Export rowaction
    /**
    ...
    public function __construct($obj, $type, $formarray = array(), $args = array())
    {
    ...
    public function get_array_val($arr, $key)
    {
    ...
    public function modarr(&$array, $boolarr, $text)
    {
        if (!is_array($array)) {
            return $array;
        }
        ...
        foreach ($array as $key => $arr) {
            if (is_array($arr)) {
                $res[$key] = $this->modarr($arr, $boolarr, $text . "[" . $key . "]");
            } else {
    ...
    public function bulk_action()
    {
        switch ($_POST['action']) {
            ...
            case "delete":{
                    ...
                    $this->xydac_core_message = $i . " " . $this->xydac_core_label . __(' Deleted.', XYDAC_CMS_NAME);
                    break;
                }
            case "activate":{
                    $i = 0;
                    ...
                    $this->xydac_core_message = $i . " " . $this->xydac_core_label . __(' Activated.', XYDAC_CMS_NAME);
                    break;
                }
            case "deactivate":{
                    $i = 0;
                    ...
                    $this->xydac_core_message = $i . " " . $this->xydac_core_label . __(' Deactivated.', XYDAC_CMS_NAME);
                    break;
                }
			...
			<!-- Main Bar  -->
				<div class="form-wrap">
					<?php do_action('xydac_core_righthead');?>
						<form id="form_edit_doaction" action="<?php if ($this->xydac_core_editmode) {
            echo $this->xydac_core_form_action . '&edit_' . $this->xydac_core_name . '=true&' . $this->xydac_core_name . '_name=' . $this->xydac_editdata[$this->namefield_name];
        } else {
            echo $this->xydac_core_form_action;
        }
        ?>" method="post">
...
echo '</div>';
	...
	<?php if (!xydac()->is_xydac_ucms_pro()) {
            xydac()->xydac_show_donate_link(false);
        }
        ?>

	<?php }

    public function print_right_table_header_footer($type = 'thead', $headfootcolumn)
    {
    ...
    public function print_right_table_body($xydac_rowdata, $headfootcolumn)
    {
        ...
        if (is_array($xydac_rowdata)) {
            foreach ($xydac_rowdata as $value) {
                ...
                ?>
				<tr id="content-type-<?php echo $name; ?>" class="<?php if (!$this->isActive($name)) {
                    echo "inactive";
                } else {
                    echo "active";
                }
...
if (!$this->xydac_core_editmode) {
    ...
    public function print_row_actions($name)
    {
        $rowactions = array();
    ...
    public function print_do_actions()
    {
    ...
    public function print_left_form()
    {
        ?>
		<form <?php if ($this->xydac_core_editmode) {
            echo "id='form_edit_" . $this->xydac_core_name . "'";
        } else {
            echo "id='form_create_" . $this->xydac_core_name . "'";
        }
        ?> action='<?php if ($this->xydac_core_editmode) {
            echo $this->xydac_core_form_action . '&edit_' . $this->xydac_core_name . '=true&' . $this->xydac_core_name . '_name=' . $this->xydac_editdata[$this->namefield_name];
        } else {
            echo $this->xydac_core_form_action;
        }
		...
		<div class="xydacfieldform">
			<h3><?php if ($this->xydac_core_editmode) {
            echo __('Edit ', XYDAC_CMS_NAME) . $this->xydac_core_label;
        } else {
            echo __('Add ', XYDAC_CMS_NAME) . $this->xydac_core_label;
        }
        ?>

			<span>
				<input type="submit"  name="xydac_form_<?php if ($this->xydac_core_editmode) {
            echo $this->xydac_core_name . '_update_submit';
        } else {
            echo $this->xydac_core_name . '_add_submit';
        }
        ?>" class="button-secondary" value="Save">
				<?php if ($this->xydac_core_editmode) {
            echo "<a class='button button-secondary'  href='" . $this->xydac_core_form_action . "&edit_" . $this->xydac_core_name . "=false" . "'>Cancel Edit</a>";
        }
        ?>
			</span>
			</h3>
			<div class="form-field form-required <?php if (isset($_POST['xydac_form_' . $this->xydac_core_name . '_update_submit']) || isset($_POST['xydac_form_' . $this->xydac_core_name . '_add_submit'])) {
            if (isset($_POST['xydac_form_' . $this->xydac_core_name][$this->namefield_name]) && empty($_POST['xydac_form_' . $this->xydac_core_name][$this->namefield_name])) {
                echo 'form-invalid';
            }
        }
        ...
        ?>"  >
				...
				<input type='text' name='xydac_form_<?php echo $this->xydac_core_name . '[' . $this->namefield_name . ']'; ?>' <?php if ($this->xydac_core_editmode) {
            echo "readonly";
        }
        ?> class='name' id='<?php echo $this->xydac_core_name . '[' . $this->namefield_name . ']'; ?>' value="<?php if ($this->xydac_core_editmode) {
            echo $this->xydac_editdata[$this->namefield_name];
        }
        ?>" />
				<p><?php echo $this->xydac_core_label;
        _e(' Name identifies your ', XYDAC_CMS_NAME);
        echo $this->xydac_core_label;
        _e(' among others. It is usually all lowercase and contains only letters, numbers, and hyphens.', XYDAC_CMS_NAME); ?></p>
			</div>
			</div>
            ...
            foreach ($this->xydac_core_form_array as $name => $data) {
                ...
                if ($type == 'heading') {
                    ...
                    if (isset($heading_desc)) {
                        echo '<p>' . $heading_desc . '</p>';
                    }
				...
				<div class='form-field <?php echo ($type == 'textarea' || !$this->xydac_core_editmode) ? '' : 'two_cols_display' ?>' id="xydac_panel_<?php echo $this->xydac_core_name . $name ?>" style="position:relative;"  >
					<?php if ($type == 'boolean') {?>
							...
							<input type='checkbox' style="width:15px;margin-left:20px" name="xydac_form_<?php echo $this->xydac_core_name . $name; ?>" id="<?php echo $this->xydac_core_name . $name; ?>" value="<?php echo $name; ?>" <?php if ($this->xydac_core_editmode && $this->get_array_val($this->xydac_editdata, $name) == 'true') {
					...
					<?php } elseif ($type == 'array') {?>
						...
						<select id='<?php echo $this->xydac_core_name . $name; ?>' name='xydac_form_<?php echo $this->xydac_core_name . $name ?>' class='postform' style="float:right;width:150px;margin-right:5%">
							<?php foreach ($values as $n => $c) {?>
								<option value='<?php echo $n; ?>' <?php if ($default == $n && !$this->xydac_core_editmode) {echo 'selected';} elseif ($this->xydac_core_editmode) {if ($n == $this->get_array_val($this->xydac_editdata, $name)) {
                        echo 'selected';
                    }
                    }?>><?php echo $c ?></option>
								<?php }?>
					...
					<?php } elseif ($type == 'checkbox') {?><label for='<?php echo $this->xydac_core_name . $name ?>' style="font-weight:bold;"><?php echo $arr_label ?></label>
						<?php $_checkboxeditdata = $this->get_array_val($this->xydac_editdata, $name);
                        $_i = 0;
                        if (!is_array($_checkboxeditdata)) {$_checkboxeditdata = array($_checkboxeditdata);}
                        foreach ($values as $val_name => $val_label) {?>
							<div style="width:180px;float:left;"><input type='checkbox' style="width:15px;margin-left:20px" name="xydac_form_<?php echo $this->xydac_core_name . $name . "[]"; ?>" id="<?php echo $this->xydac_core_name . $name; ?>" value="<?php _e($val_name, 'xydac');?>" <?php if ($this->xydac_core_editmode && in_array($val_name, $_checkboxeditdata)) {
                            echo "checked=checked";
                        }
                            ?>  />&nbsp;<?php _e($val_label, 'xydac');?></div><?php if ($_i == 1) {$_i = 0;
                                echo "<br />";} else {
                                $_i++;
                            }
                            ?>
					...
					<?php } elseif ($type == 'textarea') {?><label for='<?php echo $this->xydac_core_name . $name ?>' style="font-weight:bold;"><?php echo $arr_label ?></label>
						<textarea style="height:<?php if (isset($height)) {
                        echo $height;
                    } else {
                        echo "300px";
                    }
                        ?>" name='xydac_form_<?php echo $this->xydac_core_name . $name; ?>' class='name <?php echo $arr_clazz; ?>' id='<?php echo $this->xydac_core_name . $name; ?>'><?php if ($this->xydac_core_editmode) {echo $this->get_array_val($this->xydac_editdata, $name);}?></textarea>
					<?php }
                    $desc = trim($desc);
        ...
        ?>" class="button-primary" value="<?php if ($this->xydac_core_editmode) {
            _e('Update ' . $this->xydac_core_label, $this->xydac_core_name);
        } else {
            _e('Add ' . $this->xydac_core_label, $this->xydac_core_name);
        }
        ?>">
