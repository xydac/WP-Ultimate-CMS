...
abstract class xydac_cms_module
{
    ...
    public function __construct($module_name, $args = null)
    {
    ...
    /*-----------Filters--------------*/
    public function xydac_cms_site_style_func($style)
    {
    ...
    public function xydac_cms_site_script_func($script)
    {
    ...
    public function insert_object($type, $name, $fieldmaster, $args, $namefieldname)
    {
        if ($type == 'main' || $type == 'field') {
            ...
            return new WP_Error('err', $this->module_label . __(" Name Not allowed", XYDAC_CMS_NAME));
        }
    ...
    public function delete_object($type, $name, $fieldmaster, $namefieldname)
    {
        if ($type == 'main' || $type == 'field') {
            ...
            if ($this->dao->delete_object($opt, $name, $namefieldname)) {
                if ($type == 'main') {
                    $this->dao->delete_all_object($this->get_registered_option($type) . '_' . $name);
                }
//deleting all fields
    ...
    public function deactivate_main($name)
    {
        ...
        if (in_array($name, $xydac_active_options)) {
            foreach ($xydac_active_options as $k => $xydac_option) {
                if ($xydac_option == $name) {unset($xydac_active_options[$k]);
                    break;}
            }
        }
     ...
     */
    public function export_object($type, $namefieldname, $name = '')
    {
        if ($type != 'main') {
            return;
        }
    ...
    public function sync_object($type, $name, $namefieldname)
    {
        if ($type != 'main') {
            return;
        }
    ...
    public function get_active($args = null)
    {
    ...
    public function get_active_names()
    {
        if (!$this->uses_active || !isset($this->registered_option['active'])) {
            ...
            return $this->_get_option('active');
        }
//,array('fields'=>array('name'),'is_value_array'=>'true')
    }
    /* This function returns the array of main items */
    public function get_main($args = null)
    {
    ...
    /* This function returns the array of main items */
    public function get_main_by_name($name = null)
    {
    ...
    public function get_main_names($name = null)
    {
     ...
     */
    public function get_field($name, $args = null)
    {
    ...
    /* This function returns the array of main items */
    public function get_field_by_name($name, $field_name, $fieldname_colname = null)
    {
        if (!$this->has_custom_fields || !isset($this->registered_option['field'])) {
            return;
        }
    ...
    ^ $name: specifies the object name of which field is to be fetched.
     */
    public function get_active_fieldtypes($name)
    {
        if (!$this->has_custom_fields || !isset($this->registered_option['field'])) {
            return;
        }
    ...
    public function get_field_names($name, $fieldname_colname = null)
    {
        if (!$this->has_custom_fields || !isset($this->registered_option['field'])) {
            return;
        }
    ...
    ^ $name: specifies the object name(post_type) of which field is to be fetched.
     */
    public function get_field_type($name, $field_name, $fieldname_colname = null, $fieldtype_colname = null)
    {
        if (!$this->has_custom_fields || !isset($this->registered_option['field'])) {
            return;
        }
    ...
    public function xydac_checkbool($string)
    {
        if ($string == 'false') {
            return false;
        } else {
    ...
    public function xydac_singular($name)
    {
    ...
    public function front_header($tabs = null)
    {
        echo "<div class='wrap'>";
...
<a href="<?php echo $tab['href']; ?>"
	class="nav-tab <?php if (!empty($tab['clazz'])) {
                    echo $tab['clazz'];
                }
                ?> <?php if (($sub && $sub === $tab['name']) || (!$sub && true == $tab['default'])) {
                    echo 'nav-tab-active';
                }
                ?>"><?php echo $tab['label']; ?>
</a>
<?php
    ...
    //default tab page
    public function view_module_func($tab)
    {
        $method = 'xydac_' . $this->module_name . '_manager';
        new $method();
    }
    //help tab page - not using
    public function view_help_func($tab)
    {
    ...
    public function view_import_func($tab)
    {
    ...
    public function view_export_func($tab)
    {
        include_once dirname(__FILE__) . DS . 'includes' . DS . 'includes-tab-export.php';
    }

    //default field page
    public function view_fields_func($tab)
    {
    ...
    //default sync page
    public function view_xydac_sync_func($tab)
    {
