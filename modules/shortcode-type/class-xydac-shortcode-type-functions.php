...
class xydac_shortcode_type_functions
{
    public $shortcodes;
    public $values = array();
    public $nestedvalues = array();
    public $nestedtagname = '';
    public $text = "";
    public $nestedtagarr = array();
    public function __construct()
    {
        ...
        if (is_array($this->shortcodes) && !empty($this->shortcodes)) {
            foreach ($this->shortcodes as $shortcode) {
                ...
                if (!empty($shortcode['nestedtagname'])) {$this->nestedtagarr[] = 'xys_' . $shortcode['name'];}
                $name = 'xys_' . $shortcode['name'];
                $newshortcode[$name] = $shortcode;
            }
            add_filter('no_texturize_shortcodes', array($this, 'ignore_tag_a'));
            $this->shortcodes = $newshortcode;
        }

    }
    public function ignore_tag_a($list)
    {
        if (!empty($this->nestedtagarr)) {
            $list = array_merge($list, $this->nestedtagarr);
        }

        return $list;
    }
    public function rep_func($matches)
    {
        $val = $matches[1];
    ...
    public function get_attr_data($attr)
    {
        $attr = explode(',', $attr);
    ...
    public function xydac_shortcode($atts, $fullcontent, $shortcode_name)
    {
