...
class xydac_post_type extends xydac_cms_module
{
        ...
        if (is_array($cpts) && !empty($cpts)) {
            foreach ($cpts as $k => $cpt) {
                if ($cpt['name'] != 'post' && $cpt['name'] != 'page') {
                    ...
                    if ($this->xydac_checkbool($cpt['args']['supports']['title'])) {
                        array_push($xy_cpt['args']['supports'], 'title');
                    }

                    if ($this->xydac_checkbool($cpt['args']['supports']['editor'])) {
                        array_push($xy_cpt['args']['supports'], 'editor');
                    }

                    if ($this->xydac_checkbool($cpt['args']['supports']['author'])) {
                        array_push($xy_cpt['args']['supports'], 'author');
                    }

                    if ($this->xydac_checkbool($cpt['args']['supports']['thumbnail'])) {
                        array_push($xy_cpt['args']['supports'], 'thumbnail');
                    }

                    if ($this->xydac_checkbool($cpt['args']['supports']['excerpt'])) {
                        array_push($xy_cpt['args']['supports'], 'excerpt');
                    }

                    if ($this->xydac_checkbool($cpt['args']['supports']['trackbacks'])) {
                        array_push($xy_cpt['args']['supports'], 'trackbacks');
                    }

                    if ($this->xydac_checkbool($cpt['args']['supports']['custom-fields'])) {
                        array_push($xy_cpt['args']['supports'], 'custom-fields');
                    }

                    if ($this->xydac_checkbool($cpt['args']['supports']['comments'])) {
                        array_push($xy_cpt['args']['supports'], 'comments');
                    }

                    if ($this->xydac_checkbool($cpt['args']['supports']['revisions'])) {
                        array_push($xy_cpt['args']['supports'], 'revisions');
                    }

                    if ($this->xydac_checkbool($cpt['args']['supports']['page-attributes'])) {
                        array_push($xy_cpt['args']['supports'], 'page-attributes');
                    }
    ...
    public function get_xydac_cms_tax_combo($post_id)
    {
