...
class xydac_post_type_fields extends xydac_ultimate_cms_core
{
    ...
    public function getField($formItem, $editdata, $index)
    {
        extract($formItem);
        ...
        if ($type == 'boolean') {?>
				...
				<input type='checkbox' style="width:15px;margin-left:20px" name="xydac_form_<?php echo $formName; ?>" id="<?php echo $name; ?>" value="true" <?php if (true && $this->get_array_val($editdata, $name) == 'true') {
		...
		<?php } elseif ($type == 'array') {?>
			...
			<select id='<?php echo $name; ?>' name='xydac_form_<?php echo $formName ?>' class='postform' >
				<?php foreach ($values as $n => $c) {?>
					<option value='<?php echo $n; ?>' <?php if ($default == $n && !true) {echo 'selected';} elseif (true) {if ($n == $this->get_array_val($editdata, $name)) {
            echo 'selected';
        }
        }?>><?php echo $c ?></option>
					<?php }?>
		...
		<?php } elseif ($type == 'checkbox') {?>
				...
				<?php $_checkboxeditdata = $this->get_array_val($editdata, $name);
            $_i = 0;
            if (!is_array($_checkboxeditdata)) {$_checkboxeditdata = array($_checkboxeditdata);}
            foreach ($values as $val_name => $val_label) {?>
					<div style="width:180px;float:left;"><input type='checkbox' style="width:15px;margin-left:20px" name="xydac_form_<?php echo $formName . "[]"; ?>" id="<?php echo $name; ?>" value="<?php _e($val_name, 'xydac');?>" <?php if (true && in_array($val_name, $_checkboxeditdata)) {
                echo "checked=checked";
            }
		...
		<?php } elseif ($type == 'textarea') {?>
            ...
            ?>" name='xydac_form_<?php echo $formName; ?>' class='name' id='<?php echo $name; ?>'><?php if (true) {echo $this->get_array_val($editdata, $name);}?></textarea>
		<?php }
    }
    ...
    public function core_head($data)
    {
        ...
        ?>
		<form class="xydac_custom_field_form" <?php if ($this->xydac_core_editmode) {
        ...
        ?>" class="button-primary" value="<?php _e('Save ' . $this->xydac_core_label, $this->xydac_core_name);?>">
		</div>
...
wp_die();
    ...
    public function print_input_row($id, $name, $value = [], $readonlyName = true)
    {
